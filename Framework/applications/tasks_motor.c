...
void CMGMControlTask(void const * argument){
	while(1){
		...
		static float FrontBackSpeedTarget = 0.0;
		static float LeftRightSpeedTarget = 0.0;
		...
		if(IOPool_hasNextRead(rcUartIOPool, 0)){
			...
			FrontBackSpeedTarget = (IOPool_pGetReadData(rcUartIOPool, 0)->rc.ch1 - 1024) / 660.0 * 1000;
			LeftRightSpeedTarget = (IOPool_pGetReadData(rcUartIOPool, 0)->rc.ch0 - 1024) / 660.0 * 1000;
		}
		
		setYawWithAngle(yawAngleTarget);
		setPitchWithAngle(pitchAngleTarget);
		
		setChassisWithSpeed(FrontBackSpeedTarget, LeftRightSpeedTarget, 0);
